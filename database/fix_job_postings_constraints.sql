-- Fix job_postings table constraints and RLS policies
-- Execute this in Supabase SQL Editor

-- First, let's see what constraints exist and fix them
DROP CONSTRAINT IF EXISTS job_postings_status_check ON job_postings;

-- Add proper check constraint for status column
ALTER TABLE job_postings 
ADD CONSTRAINT job_postings_status_check 
CHECK (status IN ('active', 'closed', 'draft', 'inactive', 'archived'));
