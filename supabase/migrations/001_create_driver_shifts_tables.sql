-- =====================================================
-- CREATE DRIVER SHIFTS AND AUTOMATION TABLES
-- Based on actual database schema (uses 'staff' not 'employees')
-- =====================================================
...
-- 5. Create trip_ratings table (already exists as 'trip_ratings' in schema)
...
-- 10. Create driver_earnings table (for wallet functionality)
...
COMMENT ON TABLE driver_shifts IS 'Driver shift assignments with automated generation support';
COMMENT ON TABLE conductor_assignments IS 'Conductor assignments to trips';
COMMENT ON TABLE cleaning_requests IS 'Bus cleaning requests after trips';
COMMENT ON TABLE rating_requests IS 'Trip rating requests sent to passengers';
COMMENT ON TABLE trip_ratings IS 'Passenger ratings for trips';
COMMENT ON TABLE speed_violations IS 'Speed limit violations during trips';
COMMENT ON TABLE route_deviations IS 'Route deviation incidents';
COMMENT ON TABLE trip_reports IS 'Comprehensive trip reports';
COMMENT ON TABLE shift_generation_queue IS 'Queue for background shift generation';
COMMENT ON TABLE driver_earnings IS 'Driver earnings and wallet transactions';

-- Success message
DO $$
BEGIN
  RAISE NOTICE 'Driver Shifts and Automation tables created successfully!';
END $$;
