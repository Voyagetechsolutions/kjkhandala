...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      ...
      let allBookingsQuery = supabase.from("bookings").select("total_amount", { count: "exact" });
      let todayBookingsQuery = supabase.from("bookings").select("total_amount, payment_status").gte('created_at', todayStart).lte('created_at', todayEnd);
      let monthBookingsQuery = supabase.from("bookings").select("total_amount, payment_status").gte('created_at', monthStart).lte('created_at', monthEnd);
      let busesQuery = supabase.from("buses").select("status", { count: "exact" });
      let routesQuery = supabase.from("routes").select("*", { count: "exact" });
      let todayTripsQuery = supabase.from("trips").select("status, scheduled_departure, actual_departure").gte('scheduled_departure', todayStart).lte('scheduled_departure', todayEnd);
      let monthExpensesQuery = supabase.from("expenses").select("amount").gte('expense_date', monthStart).lte('expense_date', monthEnd);

      // Apply company_id filter if user has a company
      if (companyId) {
        allBookingsQuery = allBookingsQuery.eq('company_id', companyId);
        todayBookingsQuery = todayBookingsQuery.eq('company_id', companyId);
        monthBookingsQuery = monthBookingsQuery.eq('company_id', companyId);
        busesQuery = busesQuery.eq('company_id', companyId);
        routesQuery = routesQuery.eq('company_id', companyId);
        todayTripsQuery = todayTripsQuery.eq('company_id', companyId);
        monthExpensesQuery = monthExpensesQuery.eq('company_id', companyId);
      }
