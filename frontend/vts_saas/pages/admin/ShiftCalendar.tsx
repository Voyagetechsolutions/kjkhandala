...
export default function ShiftCalendar() {
  ...
  const createShiftMutation = useMutation({
    mutationFn: async (shiftData: any) => {
      // Find the selected route to get duration
      const selectedRoute = routes.find(r => r.id === shiftData.route_id);
      ...
      const shiftStartTime = `${shiftDate}T${shiftData.departure_time}:00`;
      ...
      const { error } = await supabase.from('driver_shifts').insert({
        ...
        shift_start_time: startDateTime.toISOString(),
        shift_end_time: endDateTime.toISOString(),
    ...
    onSuccess: () => {
      ...
      setFormData({ driver_id: '', route_id: '', bus_id: '', departure_time: '09:00' });
    },
    onError: (error: any) => {
