...
export default function OperationsDashboard() {
  const { user, userRoles, companyId, loading } = useAuth();
  const navigate = useNavigate();

  // Call useQuery hook unconditionally (before any returns)
  const { data: dashboard, isLoading } = useQuery({
    queryKey: ['operations-dashboard', companyId],
    queryFn: async () => {
      ...
      let busesQuery = supabase.from('buses').select('id, status');
      let driversQuery = supabase.from('drivers').select('id, status');
